                    ...
                    <table cellpadding="0" cellspacing="0" border="0" style="font-family: 'Century Gothic', Arial, sans-serif; margin-top: 20px;">
                      <tr>
                        <td style="padding-right: 15px; border-right: 1px solid #336699; vertical-align: top;">
                          <img src="https://canaco.net/wp-content/uploads/2024/07/logo2G.webp" alt="CANACO Monterrey" style="max-width: 150px; height: auto;">
                        </td>
    ...
    if (page && limit) {
      ...
      const paginatedQueryStr = `
        SELECT 
          ...
          (SELECT COALESCE(array_agg(usuario_id), ARRAY[]::integer[]) FROM votos_registro WHERE ticket_id = t.id) AS usuarios_votos
        FROM tickets t
...
const voteTicket = async (req, res) => {
  ...
  try {
    ...
    // 5. Emitimos evento de websockets
    const io = req.app.get('socketio');
    if (io) {
        io.emit('ticket_actualizado', result.rows[0]);
    }
    
    // 6. Devolvemos 200 OK con mensaje y ticket
    res.json({ message: "Voto registrado exitosamente", ticket: result.rows[0] });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: error.message });
  }
